...
L_thres = 0.00001
# minimum allowed friction velocity
u_friction_min = 0.01
...
def penman_monteith(T_A_K, u, ea, p, Sn, L_dn, emis, LAI, z_0M, d_0, z_u, z_T,
    ...
    T_0_K = T_A_K  # assume aerodynamic tempearture equals air temperature
...
def shuttleworth_wallace(T_A_K,
    ...
    for n_iterations in range(max_iterations):
        ...
        _, _, _, C_s, C_c = calc_effective_resistances_SW(
            R_A[i], R_x[i], R_S[i], R_c[i],
            R_ss[i], delta[i], psicr[i])
