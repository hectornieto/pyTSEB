...
def TSEB_SW_2(Tr_K,
    ...
    for n_iterations in range(max_iterations):
        ...
        beta_v = np.ones_like(Tr_K) + STEP_SEE
        see = see_max[:] + STEP_SEE
        r_c_min = pet.bulk_stomatal_resistance(LAI * f_g,
                                               Rst_min,
                                               leaf_type=leaf_type)
        ...
        while np.any(LE_S[i] < 0):
            i = np.logical_and.reduce((LE_S < 0,
                                       ...
                                       flag != F_INVALID,
                                       beta_v >= 0))
            see[i] -= STEP_SEE  # Soil is drier and hence we increase soil surface resistance
            beta_v[i] -= STEP_SEE  # Soil is drier and hence we increase soil surface resistance
            # Ensure that for almost wet soil surface T is also maximum
            # Rst[Rss <= 500] = Rst_min[Rss <= 500]

            # There cannot be negative transpiration from the vegetation
            flag[np.logical_and(i, beta_v <= 0)] = F_ZERO_LE_S
            ...
            PM_S = (delta[i] * (Rn[i] - G[i]) + (rho_cp[i] * vpd[i] - delta[i] * R_S[i] * delta_Rn[i]) / (
                    R_A[i] + R_S[i])) / \
                   (delta[i] + psicr[i] * (1. + Rss_out[i] / (R_A[i] + R_S[i])))
            PM_S[np.isnan(PM_S)] = 0
            # Eq. 11 in [Shuttleworth1985]_
            LE[i] = C_c * PM_C + C_s * PM_S
            H[i] = Rn[i] - G[i] - LE[i]
